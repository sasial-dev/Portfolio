      ...
      <div class="flex flex-col max-w-7xl justify-center mx-8 2xl:mx-0">
        <div class="flex flex-wrap sm:flex-nowrap justify-center items-center mt-16 sm:mt-12">
          <div class="text-theme-blue text-center lg:text-left">
            <h2 class="text-5xl sm:text-6xl font-black sm:mt-0">
              Hey,
            </h2>
          ...
          <div>
            <div class="relative md:w-[500px] md:h-[500px] h-full w-full">
              <canvas id="avatar" class="z-0 h-full w-full" />
              <img class="hidden sm:block absolute -z-1 top-4 left-24" src="~/assets/blob.svg">
              <img class="hidden sm:block absolute -z-1 bottom-12 right-36" src="~/assets/dots.svg">
              <button
                ...
                "
              >
                <img v-if="sasialSpinner.playing" class="w-full h-full" src="~/assets/pause.svg">
                <img v-if="!sasialSpinner.playing" class="w-full h-full" src="~/assets/play.svg">
              </button>
            </div>
          </div>
        </div>
        ...
        <div class="text-theme-blue text-center lg:text-left mt-6 lg:mt-20">
          ...
          <div class="flex flex-wrap mt-12">
            ...
            >
              ...
              <h3 class="mt-8 font-light text-2xl">
                Senior Fullstack Engineer
              </h3>
              <p class="mt-2 font-light opacity-50 text-lg">
          ...
          <div class="mt-8 flex flex-wrap gap-5 justify-center sm:justify-start items-center">
            ...
            >
              ...
              <div class="flex mt-4 justify-center items-center gap-1">
                <svg
                  v-if="presence.discord_status === 'idle'"
                ...
                <svg
                  v-if="presence.discord_status === 'online'"
                ...
                <svg
                  v-if="presence.discord_status === 'offline'"
                ...
                <svg
                  v-if="presence.discord_status === 'dnd'"
            ...
            >
              ...
              <h3 class="text-xs mt-4 opacity-50">
                slower response times than discord
              </h3>
            </div>
          </div>
        </div>
...
const presence = ref<LanyardUserData>({
  ...presenceData,
  discordStatus: statuses[presenceData.discord_status]
})

const sasialSpinner = reactive({
  playing: true,
...
onMounted(async () => {
  ...
  sasialSpinner.controls = controls
  ...
  socket.addEventListener('message', ({ data }) => {
    ...
    if (!d.heartbeat_interval) {
      presence.value = {
        ...d,
        discordStatus: statuses[d.discord_status]
      }
    }
  })
})
